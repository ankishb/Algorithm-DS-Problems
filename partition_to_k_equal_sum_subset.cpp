...
bool can_parition_k_subset(vector<int> A, int k){
	int n = A.size();
	int all_sum = 0;
	for(auto itr : A) all_sum += itr;
	if(k<=0 || k>n || all_sum%k != 0){
		return false;
	}
	int start = 0;
	int cur_bucket_sum = 0;
	int target = all_sum/k;
	vector<bool> visited(n, 0);
	return can_parition(start, A, visited, k, cur_bucket_sum, target);
}

bool can_parition(int start_idx, vector<int> A, vector<bool> &visited, int k, int cur_bucket_sum, int target){
	if(k == 1) return true;
	if(cur_bucket_sum == target){
		return can_parition(0, A, visited, k-1, cur_bucket_sum, target);
	}
	for(int i=start_idx; i<A.size(); i++){
		if(!visited[i] && cur_bucket_sum+A[i]<=target){
			visited[i] = true;
			if(can_parition(i, A, visited, k, cur_bucket_sum+A[i], target)){
				return true;
			}
			visited[i] = false;
		}
