...
// Approach:
// 1. We find the maximum deadline as m, and init m+1 set.
// 2. sort all the job with maximum profit and transverse one by one
// 3. We find the current deadline mi in parent array.
//     1. If found_index > 0, then we chose the job(add in profit), and we merge the (mi-1)th with mth set.
//     2. Else Ignore it.
